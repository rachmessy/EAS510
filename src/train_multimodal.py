...
def train_multimodal(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=5, class_names=None):
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for text_input, image_input, labels in tqdm(train_loader, desc=f"Epoch {epoch+1} [Train]"):
        ...
        with torch.no_grad():
            for text_input, image_input, labels in tqdm(val_loader, desc=f"Epoch {epoch+1} [Val]"):
                text_input = text_input.to(device)
                image_input = image_input.to(device)
                labels = labels.to(device)
        ...
        print(
            ...
            f"training loss: {avg_train_loss:.4f}, training accuracy: {train_acc:.4f}, training F1 (macro): {train_f1_macro:.4f} | "
            f"validation loss: {avg_val_loss:.4f}, validation accuracy: {val_acc:.4f}, validation F1 (macro): {val_f1_macro:.4f}"
        )

        if class_names is not None:
            print("genre F1 scores:")
